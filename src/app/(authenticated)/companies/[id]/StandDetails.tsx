  ...
  return (
    <GridList title="Stand details">
      {standDetails ? (
        <>
          <div
            className={`flex flex-col items-center justify-center text-xs ${standDetails.chairs ? "text-blue-dark" : "text-gray-300"}`}
          >
            <Armchair size={48} strokeWidth={standDetails.chairs > 0 ? 2 : 1} />
            <span>{standDetails.chairs} Chairs</span>
          </div>
          <div
            className={`flex flex-col items-center justify-center text-xs ${standDetails.table ? "text-blue-dark" : "text-gray-300"}`}
          >
            <Dock size={48} strokeWidth={standDetails.table ? 2 : 1} />
            <span>{standDetails.table ? "Want table" : "No table"}</span>
          </div>
          <div
            className={`flex flex-col items-center justify-center text-xs ${standDetails.lettering ? "text-blue-dark" : "text-gray-300"}`}
          >
            <Construction
              size={48}
